...
def accept_request(request, request_id):
    ...
    system_user, created = User.objects.get_or_create(
        username='BloodBank',
        defaults={'email': 'system@example.com', 'is_active': True}
    )

    # Send OTP via system chat
    try:
        ChatMessage.objects.create(
            sender=system_user,  # system account sends the message
            recipient=br.requester,
            content=f"OTP: {otp}\nDonor: {request.user.username}\nPhone: {donor_phone}\nAddress: {donor_address}"
        )
        messages.success(request, "✅ Donor details + OTP sent via system chat.")
    except Exception as e:
        messages.error(request, f"Chat message error: {e}")

    # Optional: send email from donor (or skip if you want system-only chat)
    try:
        send_mail(
